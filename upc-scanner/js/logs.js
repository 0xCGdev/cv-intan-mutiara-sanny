...
export async function loadLog() {
    ...
    try {
        ...
        $("logRows").innerHTML = state.logRows.length ? `<table class="table"><thead><tr><th>Waktu</th><th>UPC</th><th>Barang</th><th>Petugas</th></tr></thead><tbody>${state.logRows.map((x) => `<tr><td>${esc(x.timestamp)}</td><td>${esc(x.upc)}</td><td><b>${esc(x.name)}</b><br><span class="muted">${esc(x.sku)}</span></td><td>${esc(x.petugas)}</td></tr>`).join("")}</tbody></table>` : '<div class="empty">Belum ada log scan.</div>';
